...
```{r, echo=FALSE, include=FALSE}
	...
	require(plyr)
	require(devtools)
	source_url('https://raw.github.com/gforge/Gmisc/master/R/htmlTable.R')
	options("table_counter"=TRUE)
	subject_order <- c("READING", "MATHEMATICS", "EOC_MATHEMATICS_1", "EOC_MATHEMATICS_2")
...
```{r, cache=TRUE, results='asis', echo=FALSE, scalescores}
	t1 <- matrix(data=c(319, 318, 322, 334, 336), nrow=1)
	colnames(t1) <- c('Grade 3/2009', 'Grade 4/2010', 'Grade 5/2011', 'Grade 6/2012', 'Grade 7/2013')
	htmlTable(t1, title="", align=paste(rep('r', dim(t1)[2]), collapse=''), 
		caption='Scale scores for a hypothetical student across 5 years in reading.')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, percentilecuts}
	t2 <- matrix(data=c(304.8, 314.9, 319.9, '...',  325.9, '...', 330.8, '...',  335.5, 336.3, '...',  368.9, '...', 387.1, '...', 409.8), nrow=1)
	colnames(t2) <- c('1<sup>st</sup>', '2<sup>nd</sup>', '3<sup>rd</sup>', '...', '10<sup>th</sup>', '...', '25<sup>th</sup>', '...', '50<sup>th</sup>', '51<sup>st</sup>', '...', '75<sup>th</sup>', '...', '90<sup>th</sup>', '...', '99<sup>th</sup>')
	htmlTable(t2, title="", align=paste(rep('r', dim(t2)[2]), collapse=''), 
		caption='Percentile cutscores for grade 7 reading based upon the grade 3, 4, 5, and 6 reading scale scores given in Table <script> document.write(scalescores); </script>.')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, N_table}
	n.tbl <- table(Washington_SGP@Data[YEAR=='2012_2013' & CONTENT_AREA %in% subject_order & VALID_CASE=='VALID_CASE']$CONTENT_AREA, Washington_SGP@Data[YEAR=='2012_2013' & CONTENT_AREA %in% subject_order & VALID_CASE=='VALID_CASE']$GRADE)
	n.tbl <- n.tbl[match(subject_order, row.names(n.tbl)) ,]
	n.tbl <- cbind('Content Area'=sapply(subject_order, capwords, special.words="EOC"), n.tbl)
	n.tbl <- prettyNum(n.tbl, preserve.width = "individual",big.mark=',')
	n.tbl[which(n.tbl==0)] <- ''
	n.tbl<-n.tbl[, match(c('Content Area', sort(as.numeric(colnames(n.tbl)[-1])), 'EOCT'), colnames(n.tbl))]
	n.tbl <- n.tbl[, !is.na(colnames(n.tbl))]
	row.names(n.tbl) <- NULL
	htmlTable(as.matrix(n.tbl), title="", align=paste(rep('r', dim(n.tbl)[2]), collapse=''), 
		n.cgroup=c(1, dim(n.tbl)[2]-1), cgroup=c("", "Grades"),
		caption='Number of Valid Student Records by Grade and Subject for 2013')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, medianCohortSGPs}
	t1 <- Washington_SGP@Data[,list(Median_SGP=median(as.numeric(SGP), na.rm=TRUE)), by=c('CONTENT_AREA', 'GRADE', 'YEAR')][!is.na(Median_SGP)][YEAR=='2012_2013']
	setkey(t1)
	t1.b <- data.frame()
	y <- '2012_2013'
	if (is.null(subject_order)) subject_order <- sort(unique(t1$CONTENT_AREA))
	for (ca in subject_order){
		tmp.t1 <- data.frame(t(as.matrix(t1[YEAR==y & CONTENT_AREA==ca,][, list(Median_SGP)])))
		names(tmp.t1) <- t(as.matrix(t1[YEAR==y & CONTENT_AREA==ca,][, list(GRADE)]))
		tmp.t1 <- cbind('Content Area'=capwords(ca, special.words="EOC"), tmp.t1)
		t1.b <- rbind.fill(t1.b, tmp.t1)
	}
	t1.b <- t1.b[, match(c('Content Area', unique(c(sort(as.numeric(unique(t1$GRADE))), unique(t1$GRADE)))), names(t1.b))]
	t1.b[is.na(t1.b)] <- ""
	htmlTable(as.matrix(t1.b), title="", align=paste(rep('r', dim(t1.b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t1.b)[2]-1), cgroup=c("", "Grades"),
		caption='Median Student Growth Percentile by Grade and Content Area')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, pctAdqGrowth}
	t2 <- Washington_SGP@Data[YEAR=='2012_2013'][,list(PERCENT_CATCHING_UP_OR_KEEPING_UP=percent_in_category(CATCH_UP_KEEP_UP_STATUS, list(c('Catch Up: Yes', 'Keep Up: Yes')), list(c('Catch Up: Yes', 'Catch Up: No', 'Keep Up: Yes', 'Keep Up: No'))), PERCENT_MOVING_UP_OR_STAYING_UP=percent_in_category(MOVE_UP_STAY_UP_STATUS, list(c('Move Up: Yes', 'Stay Up: Yes')), list(c('Move Up: Yes', 'Move Up: No', 'Stay Up: Yes', 'Stay Up: No')))), by=c('CONTENT_AREA', 'GRADE')][!is.na(PERCENT_CATCHING_UP_OR_KEEPING_UP)]
	t2.b <- data.frame()
	for (u in c('CATCHING_UP', 'MOVING_UP')) {
		for (ca in c("READING", "MATHEMATICS")){
			# setkeyv(t2, names(t2)[grep(u, names(t2))])
			tmp.t2 <- data.frame(t(as.matrix(t2[CONTENT_AREA==ca,][, grep(u, names(t2)), with=FALSE])))
			names(tmp.t2) <- t(as.matrix(t2[CONTENT_AREA==ca,][, list(GRADE)]))
			tmp.t2 <- cbind('Content Area'=capwords(ca, special.words="EOC"), tmp.t2)
			t2.b <- rbind.fill(t2.b, tmp.t2)
		}
	}
	t2.b <- t2.b[, match(c('Content Area', unique(c(sort(as.numeric(unique(t2$GRADE))), unique(t2$GRADE)))), names(t2.b))]
	t2.b[is.na(t2.b)] <- ""
	htmlTable(as.matrix(t2.b), title="", align=paste(rep('r', dim(t2.b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t2.b)[2]-1), cgroup=c("", "Grades"),
		n.rgroup=c(rep(length(unique(t2$CONTENT_AREA)), 2)), rgroup= c('Catching or Keeping Up', 'Moving or Staying Up'),
		caption='Percent of Students On-Track to Attain or Maintain Adequate Growth')
``` 
...
```{r, cache=TRUE, results='asis', echo=FALSE, table3}
	t3 <- Washington_SGP@Data[YEAR=='2012_2013'][,list(Correlation =round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use="pairwise.complete.obs"), 3)), by=c('CONTENT_AREA', 'GRADE')][!is.na(Correlation)]
	t3$GRADE <- as.integer(t3$GRADE)
	setkey(t3)
	t3.b <- data.frame()
	for (ca in subject_order){
		tmp.t3 <- data.frame(t(as.matrix(t3[CONTENT_AREA==ca,][, list(Correlation)])))
		names(tmp.t3) <- t(as.matrix(t3[CONTENT_AREA==ca,][, list(GRADE)]))
		tmp.t3 <- cbind('Content Area'=capwords(ca, special.words='EOC'), tmp.t3)
		t3.b <- rbind.fill(t3.b, tmp.t3)
	}
	t3.b <- t(apply(t3.b, 1, as.character))
	t3.b[is.na(t3.b)] <- ""
	htmlTable(as.matrix(t3.b), title="", align=paste(rep('r', dim(t3.b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t3.b)[2]-1), cgroup=c("", "Grades"),
		caption='Correlations between prior standardized scale score and Cohort SGPs by Grade and Content Area')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, table4}
	sch.msgp <- Washington_SGP@Data[YEAR=='2012_2013'][, list(MEDIAN_COHORT_SGP = median(as.numeric(SGP), na.rm=T), 
		MEAN_PRIOR_SS = mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=T),
		PERCENT_PROFICIENT_PRIOR = percent_in_category(ACHIEVEMENT_LEVEL, c("L3: Proficient", "L4: Advanced"), 
			c("L1: Below Basic", "L2: Basic", "L3: Proficient", "L4: Advanced")),
		N = sum(!is.na(SGP))), by = c('SCHOOL_NUMBER', 'CONTENT_AREA')][!is.na(SCHOOL_NUMBER) & N > 9]
	t4<-sch.msgp[, list(Mean_Prior_Score = round(cor(MEDIAN_COHORT_SGP, MEAN_PRIOR_SS, use="pairwise.complete.obs"), 3),
						Percent_Prior_Proficient = round(cor(MEDIAN_COHORT_SGP, PERCENT_PROFICIENT_PRIOR, use="pairwise.complete.obs"), 3)), by=c('CONTENT_AREA')]
	setkey(t4, CONTENT_AREA)
	t4 <- t4[subject_order]
	t4$CONTENT_AREA <- sapply(t4$CONTENT_AREA, capwords, special.words='EOC')
	setnames(t4, names(t4), sapply(names(t4), capwords, special.words='EOC'))
	htmlTable(as.matrix(t4), title="", align=paste(rep('r', dim(t4)[2]), collapse=''), 
		n.cgroup=c(1, dim(t4)[2]-1), cgroup=c("", "Prior Achievement Measure"),
		caption='Correlations between school-level median SGPs and mean prior standardized scale score and percent proficient by Content Area')
```
...
```{r, cache=TRUE, results='asis', echo=FALSE, table5}
	sch.g.msgp <- Washington_SGP@Data[YEAR=='2012_2013'][, list(MEDIAN_COHORT_SGP = median(as.numeric(SGP), na.rm=T), 
		MEAN_PRIOR_SS = mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=T),
		PERCENT_PROFICIENT_PRIOR = percent_in_category(ACHIEVEMENT_LEVEL, c("L3: Proficient", "L4: Advanced"), 
			c("L1: Below Basic", "L2: Basic", "L3: Proficient", "L4: Advanced")),
		N = sum(!is.na(SGP))), by = c('SCHOOL_NUMBER', 'GRADE', 'CONTENT_AREA')][!is.na(SCHOOL_NUMBER) & N > 9]
	t5<-sch.g.msgp[, list(Mean_Prior = round(cor(MEDIAN_COHORT_SGP, MEAN_PRIOR_SS, use="pairwise.complete.obs"), 3)), by=c('CONTENT_AREA', 'GRADE')]
	setkey(t5, CONTENT_AREA, GRADE)
	t5 <- t5[subject_order]
	t5$CONTENT_AREA <- sapply(t5$CONTENT_AREA, capwords, special.words='EOC')
	t5b <- reshape(t5, direction='wide', idvar="CONTENT_AREA", timevar="GRADE")
	setnames(t5b, names(t5b), sapply(gsub('Mean_Prior.', '', names(t5b)), capwords))
	t5b <- as.matrix(t5b)[, match(c('Content Area', sort(as.numeric(names(t5b)[-1])), 'EOCT'), names(t5b), nomatch = 0)]
	t5b[is.na(t5b)] <- ""
	htmlTable(as.matrix(t5b), title="", align=paste(rep('r', dim(t5b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t5b)[2]-1), cgroup=c("", "Grades"),
		caption='Correlations between school-level median SGPs and mean prior standardized scale score by Content Area and Grade')
```
