...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	...
	setkey(t1)
```
```{r, results='asis', echo=FALSE, medianCohortSGPs}
	t1.b <- data.frame()
	y <- '2012_2013'
...
```
```{r, results='asis', echo=FALSE, pctAdqGrowth}
	t2.b <- data.frame()
	...
	htmlTable(as.matrix(t2.b), title="", align=paste(rep('r', dim(t2.b)[2]), collapse=''), 
		...
		caption='Percent of Students with Adequate Growth (On-Track to Attain or Maintain Proficiency)')
``` 
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	t4 <- Washington_SGP@Data[YEAR=='2012_2013'][,list(Correlation =round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use="pairwise.complete.obs"), 3)), by=c('CONTENT_AREA', 'GRADE')][!is.na(Correlation)]
	t4$GRADE <- as.integer(t4$GRADE)
	setkey(t4)
```
```{r, results='asis', echo=FALSE, table4}
	t4.b <- data.frame()
	for (ca in subject_order){
		tmp.t4 <- data.frame(t(as.matrix(t4[CONTENT_AREA==ca,][, list(Correlation)])))
		names(tmp.t4) <- t(as.matrix(t4[CONTENT_AREA==ca,][, list(GRADE)]))
		tmp.t4 <- cbind('Content Area'=capwords(ca, special.words='EOC'), tmp.t4)
		t4.b <- rbind.fill(t4.b, tmp.t4)
	}
	t4.b <- t4.b[, match(c('Content Area', unique(c(sort(as.numeric(unique(t4$GRADE))), unique(t4$GRADE)))), names(t4.b))]
	t4.b[is.na(t4.b)] <- ""
	htmlTable(as.matrix(t4.b), title="", align=paste(rep('r', dim(t4.b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t4.b)[2]-1), cgroup=c("", "Grades"),
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	sch.msgp <- Washington_SGP@Data[YEAR=='2012_2013'][, list(MEDIAN_COHORT_SGP = median(as.numeric(SGP), na.rm=T), 
		...
		PERCENT_PROFICIENT_PRIOR = percent_in_category(ACHIEVEMENT_LEVEL_PRIOR, c("L3: Proficient", "L4: Advanced"), 
	...
	t5<-sch.msgp[, list(Mean_Prior_Score = round(cor(MEDIAN_COHORT_SGP, MEAN_PRIOR_SS, use="pairwise.complete.obs"), 2),
						Percent_Prior_Proficient = round(cor(MEDIAN_COHORT_SGP, PERCENT_PROFICIENT_PRIOR, use="pairwise.complete.obs"), 2)), by=c('CONTENT_AREA')]
	setkey(t5, CONTENT_AREA)
	t5 <- t5[subject_order]
	t5$CONTENT_AREA <- sapply(t5$CONTENT_AREA, capwords, special.words='EOC')
	setnames(t5, names(t5), sapply(names(t5), capwords, special.words='EOC'))
	t5[is.na(t5)] <- ""
```
```{r, results='asis', echo=FALSE, table5}
	htmlTable(as.matrix(t5), title="", align=paste(rep('r', dim(t5)[2]), collapse=''), 
		n.cgroup=c(1, dim(t5)[2]-1), cgroup=c("", "Prior Achievement Measure"),
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	...
	t6<-sch.g.msgp[, list(Mean_Prior = round(cor(MEDIAN_COHORT_SGP, MEAN_PRIOR_SS, use="pairwise.complete.obs"), 2)), by=c('CONTENT_AREA', 'GRADE')]
	setkey(t6, CONTENT_AREA, GRADE)
	t6 <- t6[subject_order]
	t6$CONTENT_AREA <- sapply(t6$CONTENT_AREA, capwords, special.words='EOC')
```
```{r, results='asis', echo=FALSE, table6}
	t6b <- reshape(t6, direction='wide', idvar="CONTENT_AREA", timevar="GRADE")
	setnames(t6b, names(t6b), sapply(gsub('Mean_Prior.', '', names(t6b)), capwords))
	t6b <- as.matrix(t6b)[, match(c('Content Area', sort(as.numeric(names(t6b)[-1])), 'EOCT'), names(t6b), nomatch = 0)]
	t6b[is.na(t6b)] <- ""
	htmlTable(as.matrix(t6b), title="", align=paste(rep('r', dim(t6b)[2]), collapse=''), 
		n.cgroup=c(1, dim(t6b)[2]-1), cgroup=c("", "Grades"),
...
```{r, results='asis', echo=FALSE, scalescores}
	ex.tbl <- matrix(data=c(319, 318, 322, 334, 336), nrow=1)
	colnames(ex.tbl) <- c('Grade 3/2009', 'Grade 4/2010', 'Grade 5/2011', 'Grade 6/2012', 'Grade 7/2013')
	htmlTable(ex.tbl, title="", align=paste(rep('r', dim(ex.tbl)[2]), collapse=''), 
...
```{r, results='asis', echo=FALSE, percentilecuts}
	t3 <- matrix(data=c(304.8, 314.9, 319.9, '...',  325.9, '...', 330.8, '...',  335.5, 336.3, '...',  368.9, '...', 387.1, '...', 409.8), nrow=1)
	...
	htmlTable(t3, title="", align=paste(rep('r', dim(t3)[2]), collapse=''), 
